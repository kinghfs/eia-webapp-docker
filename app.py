...
def build_bar_trace(df: pd.DataFrame, col: str = 'Commercial Crude (Excluding SPR)') -> list[BaseTraceType]:
...
def build_line_trace(df: pd.DataFrame, col: str = 'Commercial Crude (Excluding SPR)') -> list[BaseTraceType]:
    trace = go.Scatter(x=df.index,
                       y=df[col],
                       mode='lines',
                       name=col,
...
app.layout = html.Div([
    html.H1(children='EIA Weekly Petroleum Status Report Dashboard', style={'textAlign':'center', 'fontFamily': 'sans-serif'}),
    dcc.Dropdown(inv.columns, 'Commercial Crude (Excluding SPR)', id='Dropdown', style={'fontFamily':'sans-serif'}),
...
def rescale_y_axis(figure, relay):
    ...
    if "xaxis.range[0]" in relay:
        ...
        new_layout['xaxis3']['range'] = [relay['xaxis.range[0]'], relay['xaxis.range[1]']]
